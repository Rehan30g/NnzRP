  ...
  static async render(container, activeViewName, onProxyChange) {
    ...
    container.innerHTML = `
      <div style="display:flex; align-items:center; gap:0.75rem;">
        ...
        <h2 style="font-size:1.1rem; text-transform:capitalize;">${activeViewName}</h2>
      </div>

      <div style="display:flex; align-items:center; gap:1rem;">
        <div style="display:flex; align-items:center; gap:0.5rem;">
          <span style="font-size:0.8rem; color:var(--text-muted);">Active Proxy:</span>
          <select class="select" id="header-proxy-select" style="padding:0.35rem 0.75rem; font-size:0.85rem; max-width:220px;">
            ${proxies.map(p => `
